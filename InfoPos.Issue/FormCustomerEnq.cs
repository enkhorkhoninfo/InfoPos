...
namespace InfoPos.Issue
{
    ...
    public partial class FormCustomerEnq : DevExpress.XtraEditors.XtraForm
    {
        #region[ Хувьсагч ]
        ...
        public FormCustomerEnq(InfoPos.Core.Core core, long _CustomerID, long _IssueID)
        {
            InitializeComponent();
            _core = core;
        ...
        {
            #region[ Set Value ]
            Result res = _core.RemoteObject.Connection.Call(_core.RemoteObject.User.UserNo, 205, 120001, 120001, new object[] { CustomerID, 0 });
            if (res.ResultNo == 0)
            {
            ...
            if (Static.ToInt(DRCustomer["ClassCode"]) == 1)
            {
                txtCustomerName.Text = Static.ToStr(DRCustomer["CorporateName"]);
                txtIssueCustomerName.Text = Static.ToStr(DRCustomer["CorporateName"]);
            }
            else
            {
                txtCustomerName.Text = Static.ToStr(DRCustomer["FirstName"]) + "." + Static.ToStr(DRCustomer["LastName"]);
                txtIssueCustomerName.Text = Static.ToStr(DRCustomer["FirstName"]) + "." + Static.ToStr(DRCustomer["LastName"]);
            }
            #endregion
