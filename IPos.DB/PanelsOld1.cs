...
";
        ...
        public static Result DB500007(DbConnections db, int pagenumber, int pagecount, object[] param)
        {
            ...
            try
            {
                ...
                string sql = @"
select /*+ first_rows(1) */
sm.salesno, sm.totalamount, sm.salesamount, sm.discount, sm.vat
, nvl(sp.paid,0) paid,nvl(sp.seqno,0) seqno, sm.posno, sm.cashierno
from sales sm
left join (
    select salesno,sum(payamount) paid,max(seqno) seqno 
    from salespayment
    where salesno=:1
    group by salesno
