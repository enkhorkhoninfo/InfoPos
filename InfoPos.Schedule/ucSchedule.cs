...
namespace InfoPos.Schedule
{
    public partial class ucSchedule : UserControl
    {
        ...
        private void schedulerControl1_EditAppointmentFormShowing(object sender, DevExpress.XtraScheduler.AppointmentFormEventArgs e)
        {
            ...
            CustomAppBack myForm = new CustomAppBack(_core, (SchedulerControl)sender, apt, openRecurrenceForm, OrderNo, ProdNo, ProdType, CustName);
                try
                {
        ...
        public Result RefreshData(int ProdType,string ProdNo,DateTime Today)
        {
            ...
            {
                schedulerStorage1.Appointments.DataSource = null;
                res = _core.RemoteObject.Connection.Call(_core.RemoteObject.User.UserNo, 210, 130121, 130121, new object[] { ProdType, ProdNo, Today.ToShortDateString(), Static.ToDateTime("0001.01.01") });
                if (res.ResultNo == 0)
                {
                    DataTable dt = new DataTable();
                    dt = res.Data.Tables[0];
