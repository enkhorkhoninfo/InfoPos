    ...
    public partial class frmTemp : Form
    {
        ...
        void PostSalesToGL()
        {
            ...
            try
            {
                ...
                foreach (DataRow row in TodaySales.Rows)
                {
                    ...
                    vDiscount = Static.ToDecimal(row["Discount"]);
        ...
        ", conn);
                            ...
                            sql = " INSERT INTO dbo.tblARInvoices(ARInvoiceID,OrderID,SrbsPaymentId,Invoice#,Batch#,Ref#,Date,ARInvoiceDate,DebitGLID,Amount,ARBalance,CustomerID,Author,PO#,JobID,SalesmanID,DocType,ActivityID)" +
                                  " VALUES(" + vGetNextARID + ",0," + vSalesNo + ",'" + vGetNextARID + "'," + vGetNextBatch + ",'" + vSalesNo + "'," + datestr + "," + datestr + "," + vARAccountNo + "," + vTxnAmount + "," + vTxnAmount + "," + vCustNo + ",'Author','" + vSalesNo + "',0," + vCashierNo + ",1,0)";
                            myAccessCommand = new OleDbCommand(sql, conn);
                            //myAccessCommand.Transaction = myTrans;
                            result = myAccessCommand.ExecuteNonQuery();
                        ...
                        case 1:
                            ...
                            sql = " INSERT INTO dbo.tblARInvoiceDetail (ARInvoiceID,SrbsPaymentDetailId,CreditGLID,UnitPrice,Quantity,SplitAmount,Description,Type,ActivityID, Discount)" +
                                  " VALUES (" + vGetNextARID + "," + vSeqNo + "," + vARAccountNo + "," + vTxnAmount + ",1," + vTxnAmount + ",'" + vDesc + "',14,0,"+vDiscount+")";
                            myAccessCommand = new OleDbCommand(sql, conn);
                            //myAccessCommand.Transaction = myTrans;
                            result = myAccessCommand.ExecuteNonQuery();
                            #endregion
        ...
        ", conn);
                            ...
                            sql = "UPDATE dbo.tblARInvoices set ARBalance = ARBalance-" + vTxnAmount + " where ARInvoiceID=" + vGetNextARID;
                            myAccessCommand = new OleDbCommand(sql, conn);
                            //myAccessCommand.Transaction = myTrans;
                            result = myAccessCommand.ExecuteNonQuery();
                            #endregion
        ...
        void PostCashierToGL()
        {
            Result res = new Result();
            DataTable TodayShift;
            OleDbCommand myAccessCommand = null;
            OleDbDataAdapter myDataAdapter = null;
            OleDbDataReader reader = null;
            OleDbTransaction myTrans = null;
            ...
            try
            {
                ...
                #region [ Өнөөдөр борлуулалтын гүйлгээнүүд ]
                res = _core.RemoteObject.Connection.Call(_core.RemoteObject.User.UserNo, 607, 4, 4, new object[] { txtTxnDate.Text.Trim() });
                if (res.ResultNo != 0)
                {
                    MessageBox.Show(res.ResultDesc, "Алдаа", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    Close();
                }
                TodayShift = res.Data.Tables[0];
                #endregion

                myTrans = conn.BeginTransaction();
                foreach (DataRow row in TodayShift.Rows)
                {
                    vARAccountNo = Static.ToStr(row["DebitAcntNo"]);
                    vSalesNo = Static.ToStr(row["UserNo"]) + Static.ToDate(row["TxnDate"]).ToString("yyyyMMdd");
                    vTxnDate = Static.ToDate(row["TxnDate"]);
                    vCustNo = Static.ToStr(row["UserNo"]);
                    vCashierNo = Static.ToStr(row["UserNo"]);
                    vTxnAmount = Static.ToDecimal(row["Debt"]);
                    vDesc = "Илүүдэл дутагдал";
        ...
        ", conn);
                    ...
                    progress += 1;
                    txtProgress.Value = progress * 100 / TodayShift.Rows.Count;
                }
                listBox1.Items.Add("FINISH Invoice information loading...");
                resultTran = 1;
            }
